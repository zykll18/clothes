    ...
    <div className="group relative w-full">
      ...
      >
        <div className="pointer-events-none absolute inset-x-8 top-7 flex items-center justify-between text-[10px] uppercase tracking-[0.3em] text-[rgba(255,245,225,0.45)] sm:text-[11px]">
          ...
          <span>{previewUrl ? 'Replacement Ready' : 'Upload Required'}</span>
        </div>
        ...
        {previewUrl ? (
          <div className="relative flex h-full min-h-[26rem] flex-col justify-end sm:min-h-[30rem]">
            ...
            <div className="relative z-10 mt-auto flex flex-col gap-5 bg-[linear-gradient(180deg,transparent_0%,rgba(4,4,4,0.78)_34%,rgba(4,4,4,0.92)_100%)] px-7 pb-8 pt-20 sm:px-8">
              ...
              <div className="flex flex-col gap-3 text-[rgba(255,245,225,0.88)] sm:flex-row sm:items-center sm:justify-between">
                <div className="inline-flex items-center gap-3 text-sm">
                  ...
                  <span>点击更换图片，保留当前流程进度</span>
                </div>

                <span className="inline-flex items-center gap-2 self-start rounded-full border border-[rgba(255,255,255,0.14)] bg-[rgba(255,255,255,0.05)] px-4 py-2 text-xs uppercase tracking-[0.22em] text-[rgba(255,245,225,0.82)] transition group-hover:border-[rgba(212,177,106,0.34)] group-hover:text-white sm:self-auto">
                  <Upload size={14} />
                  Replace Image
                </span>
              </div>
          ...
          <div className="relative flex min-h-[26rem] flex-col justify-between px-7 pb-8 pt-20 sm:min-h-[30rem] sm:px-8 sm:pb-9">
            <div className="grid gap-8 lg:grid-cols-[minmax(0,1.1fr)_14rem] lg:items-end">
              <div>
            ...
            <div className="mt-10 flex flex-1 items-center justify-center">
              <div className="flex w-full max-w-xl flex-col items-center text-center">
                ...
                <p className="mt-8 text-sm uppercase tracking-[0.3em] text-[rgba(255,245,225,0.62)]">Tap To Begin</p>
                <p className="mt-4 max-w-md text-sm leading-7 text-[var(--lux-muted-foreground)]">
                  选择清晰图片进入这一幕。上传后将直接替换当前展示内容，不影响既有步骤逻辑。
                </p>
              </div>
            </div>
          </div>
